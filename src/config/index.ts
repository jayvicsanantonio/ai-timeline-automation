...
const EnvSchema = z.object({
  ...
  SIGNIFICANCE_THRESHOLD: z.string().transform(Number).pipe(z.number().min(0).max(10)).default('7.0'),
  NEWS_SOURCES: z.string().default('hackernews,arxiv,rss'),
  LOG_LEVEL: z.enum(['error', 'warn', 'info', 'debug']).default('info'),
  DRY_RUN: z.enum(['true', 'false']).transform(v => v === 'true').default('false'),
  ...
  load(): AppConfig {
    ...
    try {
      ...
      this.config = {
        ...
        newsSources: this.env.NEWS_SOURCES.split(',').map(s => s.trim()).filter(Boolean),
